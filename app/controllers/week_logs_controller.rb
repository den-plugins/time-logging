class WeekLogsController < ApplicationController
    ...
    if params[:load_type] == "project"
      ...
      case params[:proj_dir]
        when "" then
          params[:proj_dir] = "asc"
        when "asc" then
          params[:proj_dir] = "desc"
        when "desc" then
          params[:proj_dir] = "asc"
      end
      if proj_cache && !proj_cache.empty?
        @issues[:project_related] = Issue.find(proj_cache)
      else
        @issues[:project_related] = Issue.in_projects(@projects[:non_admin]).all(:conditions => ["updated_on > ?", Date.today - 6.months], :order => "#{Issue.table_name}.project_id DESC, #{Issue.table_name}.updated_on DESC")
  ...
  def sort(array, column, direction, tracker, proj_name)
    ...
    if column
      case column
        when 'task_activity' then
          array = array.sort_by { |i| i.id }
      end
    end
